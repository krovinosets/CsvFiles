            ...
            while (getline(fp,line)){
                ...
                if (strcmp(array[fa->region_index_at_header],fa->region)==0){
                    *(data_for_chosen_region+count_lines)=array;
                    count_lines++;
                    if ( (array[fa->column] != NULL) && (strcmp(array[fa->column],"")!=0) && (!isalpha(*(array[fa->column]))) ){
                        count_ocur_lines++;
                ...
                frv->liveData = (double*) malloc(vectorForMedian.size()*sizeof(double));
                frv->lenData = vectorForMedian.size();
                for(size_t i = 0; i < vectorForMedian.size(); i++){
                    frv->liveData[i] = vectorForMedian[i];
                }
            }
            if(count_ocur_lines == 0)
